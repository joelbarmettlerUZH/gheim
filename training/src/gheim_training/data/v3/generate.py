...
Output layers (data/layer_v3_*.jsonl):
  - layer_v3_emails.jsonl       — greeting + body[+] + signature compositions
  - layer_v3_docs.jsonl         — DE legal/medical/bank/HR document templates
  - layer_v3_short_form.jsonl   — standalone narrative fragments
  - layer_v3_forms.jsonl        — form/list/CSV-style high-density chunks
  - layer_v3_common_word.jsonl  — common-word surname pos+neg pairs
  - layer_v3_adversarial.jsonl  — adversarial-negative chunks (no PII)
...
Run
---
    uv run python -m gheim_training.data.v3.generate
"""
...
def _load(module_name: str) -> list[Fragment]:
    ...
    import importlib
    m = importlib.import_module(f"gheim_training.data.v3.templates.{module_name}")
...
def _load_common_word() -> tuple[list[Fragment], dict[str, list[tuple[str, str]]]]:
    ...
    m = importlib.import_module(
        "gheim_training.data.v3.templates.common_word_de",
    )
...
def _gen_docs(rng: random.Random) -> list[Chunk]:
    ...
    for tpl in all_docs:
        for _ in range(DOC_RENDERS_PER_TEMPLATE):
            chunk = render_standalone(tpl, source="synthetic_v3_docs", rng=rng)
...
def _gen_short_form(rng: random.Random) -> list[Chunk]:
    ...
    for tpl in all_sf:
        for _ in range(SHORT_FORM_RENDERS_PER_TEMPLATE):
            chunk = render_standalone(tpl, source="synthetic_v3_short_form",
                                      rng=rng)
            chunk = maybe_apply_noise(chunk, probability=NOISE_PROBABILITY,
                                      rng=rng)
            out.append(chunk)
...
def _gen_forms(rng: random.Random) -> list[Chunk]:
    ...
    for tpl in forms:
        for _ in range(FORM_RENDERS_PER_TEMPLATE):
            chunk = render_standalone(tpl, source="synthetic_v3_forms", rng=rng)
            chunk = maybe_apply_noise(chunk, probability=NOISE_PROBABILITY,
                                      rng=rng)
            out.append(chunk)
    return out
    ...
    for tpl in templates:
        for _ in range(COMMON_WORD_RENDERS_PER_TEMPLATE):
            ...
            chunk = Chunk(
                ...
                language=tpl.language, source="synthetic_v3_common_word",
    ...
    for tpl in advs:
        for _ in range(ADVERSARIAL_RENDERS_PER_TEMPLATE):
            chunk = Chunk(
                ...
                source="synthetic_v3_adversarial",
                template_id=tpl.template_id,
                meta={"polarity": "neg"},
            )
            chunk = maybe_apply_noise(chunk, probability=NOISE_PROBABILITY,
...
def main() -> None:
    ...
    if "emails" not in args.skip:
        ...
        out = args.out_dir / "layer_v3_emails.jsonl"
    ...
    if "docs" not in args.skip:
        ...
        out = args.out_dir / "layer_v3_docs.jsonl"
    ...
    if "short_form" not in args.skip:
        ...
        out = args.out_dir / "layer_v3_short_form.jsonl"
        n = write_jsonl(out, chunks)
        print(f"  wrote {n:,} → {out}")
    ...
    if "forms" not in args.skip:
        ...
        out = args.out_dir / "layer_v3_forms.jsonl"
        n = write_jsonl(out, chunks)
        print(f"  wrote {n:,} → {out}")
    ...
    if "common_word" not in args.skip:
        ...
        out = args.out_dir / "layer_v3_common_word.jsonl"
        n = write_jsonl(out, chunks)
        print(f"  wrote {n:,} → {out}")
    ...
    if "adversarial" not in args.skip:
        ...
        out = args.out_dir / "layer_v3_adversarial.jsonl"
        n = write_jsonl(out, chunks)
        print(f"  wrote {n:,} → {out}")
