...
Reads ``data/v2_balanced.jsonl`` and writes three split files plus a
...
Outputs
-------
- ``data/v2_balanced_train.jsonl``  (~80% of chunks)
- ``data/v2_balanced_val.jsonl``    (~10%)
- ``data/v2_balanced_test.jsonl``   (~10%)
- ``data/v2_split_summary.json``    per-split + per-(lang × cat) counts

Run
---
    uv run python -m gheim_training.data.v2.split
"""
