...
ASSEMBLED_PATH = Path("data/v3_assembled.jsonl")  # v3-3 produced this from v2_assembled
...
V3_EMAILS_PATH = Path("data/layer_v3_emails.jsonl")
V3_DOCS_PATH = Path("data/layer_v3_docs.jsonl")
V3_SHORT_FORM_PATH = Path("data/layer_v3_short_form.jsonl")
V3_FORMS_PATH = Path("data/layer_v3_forms.jsonl")
V3_COMMON_WORD_PATH = Path("data/layer_v3_common_word.jsonl")
V3_ADVERSARIAL_PATH = Path("data/layer_v3_adversarial.jsonl")
OUT_PATH = Path("data/v3_balanced.jsonl")
...
SYNTHETIC_SOURCES = (
    ...
    # v3 layers
    "synthetic_v3_emails", "synthetic_v3_docs", "synthetic_v3_short_form",
    "synthetic_v3_forms", "synthetic_v3_common_word",
    "synthetic_v3_adversarial",
)
...
def _phase_d_negatives(
    ...
    Two-tier emission:
    - Tier 1: ALL synthetic_v3_adversarial chunks are admitted (these are
    ...
    for c in chunks:
        ...
        if c.subset == "synthetic_v3_adversarial":
...
def main() -> None:
    ...
    syn_v3_emails = _load_synthetic_metadata(
        V3_EMAILS_PATH, "synthetic_v3_emails",
    )
    print(f"  v3 emails: {len(syn_v3_emails):,} chunks")
    print(f"[Pass 1] Loading {V3_DOCS_PATH} …")
    syn_v3_docs = _load_synthetic_metadata(V3_DOCS_PATH, "synthetic_v3_docs")
    ...
    syn_v3_short_form = _load_synthetic_metadata(
        V3_SHORT_FORM_PATH, "synthetic_v3_short_form",
    )
    print(f"  v3 short_form: {len(syn_v3_short_form):,} chunks")
    print(f"[Pass 1] Loading {V3_FORMS_PATH} …")
    syn_v3_forms = _load_synthetic_metadata(V3_FORMS_PATH, "synthetic_v3_forms")
    ...
    syn_v3_common_word = _load_synthetic_metadata(
        V3_COMMON_WORD_PATH, "synthetic_v3_common_word",
    )
    ...
    syn_v3_adversarial = _load_synthetic_metadata(
        V3_ADVERSARIAL_PATH, "synthetic_v3_adversarial",
    )
    ...
    syn_v3_ids = {
        "synthetic_v3_emails": {c.id for c in syn_v3_emails if c.id in admitted_all},
        "synthetic_v3_docs": {c.id for c in syn_v3_docs if c.id in admitted_all},
        "synthetic_v3_short_form": {c.id for c in syn_v3_short_form if c.id in admitted_all},
        "synthetic_v3_forms": {c.id for c in syn_v3_forms if c.id in admitted_all},
        "synthetic_v3_common_word": {c.id for c in syn_v3_common_word if c.id in admitted_all},
        "synthetic_v3_adversarial": {c.id for c in syn_v3_adversarial if c.id in admitted_all},
    }
    ...
    print(f"  v3 emails: {len(syn_v3_ids['synthetic_v3_emails']):,}  "
          f"docs: {len(syn_v3_ids['synthetic_v3_docs']):,}  "
          f"short_form: {len(syn_v3_ids['synthetic_v3_short_form']):,}  "
          f"forms: {len(syn_v3_ids['synthetic_v3_forms']):,}  "
          f"common_word: {len(syn_v3_ids['synthetic_v3_common_word']):,}  "
          f"adversarial: {len(syn_v3_ids['synthetic_v3_adversarial']):,}")
    ...
    def _generate() -> Iterator[V2Example]:
        ...
        # v3 synthetic layers
        v3_paths = {
            "synthetic_v3_emails": V3_EMAILS_PATH,
            "synthetic_v3_docs": V3_DOCS_PATH,
            "synthetic_v3_short_form": V3_SHORT_FORM_PATH,
            "synthetic_v3_forms": V3_FORMS_PATH,
            "synthetic_v3_common_word": V3_COMMON_WORD_PATH,
            "synthetic_v3_adversarial": V3_ADVERSARIAL_PATH,
        }
