...
\texttt{data/p7\_audit\_*.jsonl} so the 0.71 number is reproducible
from the released sample.
...
the model's headline 0.916 strict F\textsubscript{1} should be read
against this with that caveat. An earlier P2 pilot audit (n=176,
single Qwen re-annotation) gave F\textsubscript{1}=0.66 and was
used during dataset development as a coarse quality gate; the 0.71
number above is the reproducible, multi-model successor.
...
\begin{itemize}
  \item \textbf{Regex-layer false positives in
        ...
        models reject them. \emph{This is a dataset bug a v2
        cleanup pass should fix} (e.g.\ require column-context
        before flagging IBAN-shaped numerics).
  \item \textbf{Recall-aggressive labelling of court roles}
        ...
        makes by design, but a v2 could expose it as an opt-in
  ...
  \item \textbf{Under-recall of names in commercial-register
        ...
        recall gap a v2 should fix} by re-labelling the
        commercial-register subset with a prompt that
        emphasises this format.
\end{itemize}
The artefacts that produced these numbers (raw per-model
labellings, consensus, classification breakdown, example chunks)
are committed under \texttt{data/p7\_audit\_*.jsonl},
\texttt{data/p7\_audit\_report.json} and
\texttt{data/p7\_audit\_analysis.json}, and the scripts that
produced them are at
\texttt{training/src/gheim\_training/data/p7\_audit\_*.py}.
...
dataset-cleanup tasks for a v2 revision: (a) re-filtering the
regex-layer \texttt{account\_number} hits to require column-context
before flagging IBAN/AHV-shaped numerics, (b) optionally exposing
the recall-aggressive policy on court-document roles as an opt-in
flag rather than the default, and (c) re-labelling the
commercial-register subset with a prompt that emphasises the
contact-block format so director and board-member names stop being
under-recalled.
