...
\paragraph{Inference-time suppression, browser quantisation, and the calibrated decoder.}
...
identifies bias 0.5 as the Pareto-clean point on both the fp32
PyTorch backend and the int8 quantised ONNX deployment target.

On fp32 the bias has essentially no effect on the probe (91.5\%
perfect at both 0.0 and 0.5) and gains 0.27~pp char
F\textsubscript{1} on the slice at zero cost. The dataset pipeline
described in Section~\ref{sec:dataset} (gazetteer-driven address
labelling, overlap resolution, multi-LLM consensus) appears to
have absorbed most of the original suppression pathology that
motivated the wrapper; the wrapper is retained as the SDK default
because it remains net-positive rather than because it is doing
substantial pathology repair. The more interesting finding
concerns the int8 browser deployment
(Section~\ref{subsec:deployment}). On the same probe, the int8
model drops from 91.5\% perfect at bias 0.0 to 73.1\% perfect at
bias 0.0 — an 18.4~pp gap that does not appear in the
in-distribution test-set numbers reported in
Section~\ref{subsec:deployment}, where int8 costs only 0.13~pp
char F\textsubscript{1}. Quantisation appears to perturb the
\texttt{O}-versus-entity margin specifically on long-tail edge
cases while leaving mainstream chunks essentially unaffected.
The calibrated decoder partially recovers this gap: on int8 it
lifts the probe perfect-rate by 1.4~pp (73.1\% $\to$ 74.5\%) and
slice char F\textsubscript{1} by 0.43~pp, roughly $3\times$ the
fp32 lift. Browser users therefore see a larger calibration win
than server-side fp32 users; neither backend incurs a cost at
bias 0.5, and higher bias values (1.0--1.5) trade modest probe
gains for slice F\textsubscript{1} losses and are not recommended.
The wrapper is the default detector in both the Python and
JavaScript SDKs.
