...
four independent LLM subagents using a different prompt and a
different seed for each, and the four labellings were combined
into a per-span majority vote (a span is in the consensus iff
${\geq}2$ of 4 subagents agreed on it). The released dataset's
labels reach an F\textsubscript{1} of \textbf{0.48} against that
majority vote, with high per-category variance: structured
categories where the surface form carries the signal score well
(\texttt{private\_phone} 0.95, \texttt{private\_url} 0.86,
\texttt{private\_email} 0.71), while categories whose label
depends on a policy interpretation score poorly
(\texttt{private\_address} 0.12, \texttt{private\_person} 0.36,
\texttt{private\_date} 0.43). Inspection of the
...
modal subagent decision is to omit publicly-listed officials,
authors of court rulings, and witness names, while the
recall-oriented dataset policy includes them. \texttt{private\_date}
disagreements are mostly subagents extracting fewer dates from
multi-date documents (court hearings, parliamentary records).
The 0.48 audit number is therefore best read as a measure of
\emph{how aggressively the dataset labels relative to a more
conservative subagent consensus}, not as a label-noise floor in
the random-error sense; the model's headline 0.916 strict
F\textsubscript{1} should be read against this with that caveat.
The smaller \texttt{p2} pilot audit (n=176, single Qwen
re-annotation) gave F\textsubscript{1}=0.66 on a similar setup
and was used during dataset development as a coarse quality
gate; it is not the four-way number quoted above.
...
Epochs                & 3 max; best at step 2{,}500 of 3{,}603 (epoch 2.08) \\
...
inference. The int8 export reproduces 99.4\% of the fp32 strict
...
selection. The full per-language $\times$ per-category char
F\textsubscript{1} breakdown is in
...
0.884 (de), 0.862 (fr), 0.810 (it), 0.795 (rm).
...
\begin{itemize}
  ...
  \item \textbf{Annotations are machine-generated.} The dataset
        was not human-annotated; the four-way subagent audit in
        Section~\ref{sec:labelnoise} (F\textsubscript{1}~0.48
        overall, with policy-interpretation disagreements
        dominating the gap) is the only quantitative
        labelling-policy reference we can quote.
...
\textsf{gheim-ch-pii-171k}. Structured categories with strong surface
...
on the \textsf{gheim-ch-pii-171k} test split under the same harness.
...
\begin{table*}[!htbp]
\centering
\caption{Other models on the \textsf{gheim-ch-pii-171k} test split.
