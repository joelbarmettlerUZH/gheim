...
(\texttt{CalibratedDetector}, default bias 0.5). On a 212-case
forensic edge-case probe combined with a 300-chunk in-distribution
test slice, a sweep over $\{0.0, 0.25, 0.5, 0.75, 1.0, 1.5\}$
identifies bias 0.5 as the Pareto-clean point: the probe
perfect-rate is essentially flat (91.5\% at both 0.0 and 0.5) and
slice char F\textsubscript{1} gains 0.27~pp at zero cost. The
dataset pipeline described in Section~\ref{sec:dataset}
(gazetteer-driven address labelling, overlap resolution, multi-LLM
consensus) appears to have absorbed most of the original
suppression pathology that motivated the wrapper; the wrapper is
retained as the default detector in both SDKs because it remains
net-positive rather than because it is doing substantial pathology
repair. Higher bias values (1.0--1.5) trade modest probe gains for
slice F\textsubscript{1} losses and are not recommended.

\paragraph{A runtime-specific failure of the int8 browser path.}
A separate diagnostic comparing the same shipped
\texttt{model\_quantized.onnx} file under Python
\texttt{onnxruntime} versus \texttt{transformers.js} on
\texttt{onnxruntime-web} surfaced a sharp runtime gap that we
initially mis-attributed to int8 quantisation itself. Running the
212-case probe through Python with the CPU execution provider gives
fp32 91.5\%, fp16 91.5\%, int8 91.0\% perfect-rates --- the int8
file is essentially fp32-equivalent on the reference kernels.
Running the same int8 file through \texttt{transformers.js} on
Node.js WASM drops the perfect-rate to 73.1\%, and two specific
Swiss patterns that pass Python int8 (common-word surnames like
``Bach''; commercial-register person listings such as ``\textit{Beat
Aregger}'') become four-way false positives and complete misses
respectively. The shipped int8 weights are sound. The
\texttt{onnxruntime-web} WASM kernel that consumes them, the
\texttt{transformers.js} aggregation logic, or the JS tokenizer
implementation is the failure mode; we did not isolate which.
Because we control the model card but not the JS runtime, we
re-published the model with a third ONNX variant
(\texttt{model\_fp16.onnx}, byte-equivalent to fp32 on the probe)
and changed the JS SDK default to prefer fp16 when WebGPU is
available, with the int8 file retained as the explicit low-RAM
fallback. The published Python SDK is unaffected; users running
\texttt{ORTModelForTokenClassification} on the int8 file get the
correct 91.0\% perfect-rate that all this paper's quoted int8
numbers (Section~\ref{subsec:deployment}) reflect.
